...
@router.put("/{call_id}", response_model=VideoCallRead)
def update_call(
    *,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_active_user),
    ...
    if current_user.id != call.creator_id and current_user.role != "staff":
        raise HTTPException(
            status_code=403,
    ...
    for field, value in call_in.model_dump(exclude_unset=True).items():
        if field == "recording_enabled" and current_user.role == "attorney":
            continue  # Skip recording changes for legal calls
...
def cancel_call(
    *,
    ...
    # Only creator or staff can cancel
    if current_user.id != call.creator_id and current_user.role != "staff":
        raise HTTPException(
            status_code=403,
            detail="Not enough permissions"
